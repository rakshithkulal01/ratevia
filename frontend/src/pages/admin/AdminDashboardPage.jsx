...
export const AdminDashboardPage = () => {
  ...
  const handleProvisionSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        setActionSuccess(`Business "${provisionName}" provisioned successfully!`);
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1">
                Provision new businesses manually after ₹1,000 one-time payment, and manage active/suspended account statuses.
              </p>
            </div>
            <div className="flex items-center gap-3">
              <Button variant="primary" size="sm" onClick={() => setShowProvisionModal(true)}>
      ...
      <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8 pt-8 space-y-8">
        ...
        <div className="space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div>
              <h2 className="text-lg font-semibold text-foreground">Provisioned Businesses</h2>
              <p className="text-xs text-muted-foreground">Directory of registered venues and active review intake statuses</p>
            </div>
          ...
          {loading ? (
            ...
            <Card className="p-12 text-center bg-muted/20">
              ...
              <p className="text-xs text-muted-foreground mt-1">
                {searchQuery ? 'Try adjusting your search filter' : 'No businesses provisioned yet. Click "Provision Business" to add one.'}
              </p>
            </Card>
          ) : (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-xs">
          <Card className="max-w-lg w-full p-6 shadow-2xl space-y-4">
            <div className="flex items-center justify-between border-b border-border pb-3">
              <div>
                <h3 className="font-semibold text-foreground text-lg">Provision New Business</h3>
                <p className="text-xs text-muted-foreground">Manual ₹1,000 one-time business onboarding</p>
              </div>
              <button
